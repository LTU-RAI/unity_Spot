...
class QuadrupedController:
    def __init__(self):
        # Change between walking or stair mode
        self.mode = rospy.get_param("~mode", "walking")
        if self.mode == "walking":
            # Const Gait parameters
            self.Tswing = 0.22 # 0.17
            self.ClearanceHeight = 0.17 # 0.15
            self.PenetrationDepth = 0.00001 # 0.00001
        elif self.mode == "stairs":
            # Const Gait parameters
            self.Tswing = 0.40
            self.ClearanceHeight = 0.19 # 0.15
            self.PenetrationDepth = 0.00001 # 0.00001
        ...
        self.time_velocity = 0.01

        # Limits
        self.max_linear_velocity_x = 2.1   # m/sec
        self.max_linear_velocity_y = 1.0  # m/sec
        ...
        self.time_stop = rospy.get_param("~time_stop", self.Tswing)
        rospy.Timer(rospy.Duration(0.6), self.check_velocity_update_time)
